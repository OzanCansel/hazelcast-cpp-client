...
operator<<(std::ostream& os, ClientConnectionManagerImpl::client_state state)
{
    ...
    switch (state)
    {
        case client_state::INITIAL: return os << "INITIAL";
        case client_state::CONNECTED_TO_CLUSTER: return os << "CONNECTED_TO_CLUSTER";
        case client_state::INITIALIZED_ON_CLUSTER: return os << "INITIALIZED_ON_CLUSTER";
        case client_state::DISCONNECTED_FROM_CLUSTER: return os << "DISCONNECTED_FROM_CLUSTER";
        case client_state::SWITCHING_CLUSTER : return os << "SWITCHING_CLUSTER";
    }
...
void
ClientConnectionManagerImpl::initialize_client_on_cluster(boost::uuids::uuid target_cluster_id)
{
    try
    {
        {
    ...
    catch (const std::exception& e)
    {
        ...
        auto cluster_name = client_.get_client_config()
                                   .get_cluster_name();
...
ClientConnectionManagerImpl::check_invocation_allowed()
{
    ...
    if (state == client_state::INITIALIZED_ON_CLUSTER && active_connections_.size() > 0) {
        return;
    }
