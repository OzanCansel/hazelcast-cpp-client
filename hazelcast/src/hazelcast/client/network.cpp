...
std::ostream& operator<<(std::ostream& os, const socket& channel)
{
    return os << "Socket {"
              << channel.local_socket_address()
              << "->"
...
authentication_response authentication_response::from(protocol::ClientMessage response)
{
    switch (response.get_message_type()){
        ...
        case EXPERIMENTAL_AUTHENTICATION_CUSTOM_CODEC_RESPONSE_MESSAGE_TYPE:
            return from_experimental_authentication_custom_codec(std::move(response));
    }

    BOOST_THROW_EXCEPTION(exception::illegal_state{"Unexpected response message type"});
}

byte authentication_response::status() const
{
    return status_;
}

boost::optional<address> authentication_response::addr() const
{
    return server_address_;
}

boost::uuids::uuid authentication_response::member_uuid() const
{
    return member_uuid_;
}

byte authentication_response::serialization_version() const
{
    return serialization_version_;
}

const std::string& authentication_response::server_hazelcast_version() const
{
    return server_version_;
}

int authentication_response::partition_count() const
{
    return partition_count_;
}

boost::uuids::uuid authentication_response::cluster_id() const
{
    return cluster_id_;
}

const std::vector<int>& authentication_response::tpc_ports() const
{
    return tpc_ports_;
}

authentication_response authentication_response::from_authentication_codec(protocol::ClientMessage response)
{
    return decode(std::move(response), false);
}

authentication_response authentication_response::from_authentication_custom_codec(protocol::ClientMessage response)
{
    return decode(std::move(response), false);
}

authentication_response authentication_response::from_experimental_authentication_codec(protocol::ClientMessage response)
{
    return decode(std::move(response), true);
}

authentication_response authentication_response::from_experimental_authentication_custom_codec(protocol::ClientMessage response)
{
    return decode(std::move(response), true);
}

authentication_response authentication_response::decode(protocol::ClientMessage response, bool with_tpc_ports)
{
    auto* initial_frame =
    ...
    switch (credential->type()) {
        ...
        case security::credentials::credential_type::token: {
            if (is_tpc_aware_client_){
                ...
                return protocol::codec::experimental_authenticationcustom_encode(
                    cluster_name,
                    cr->token(),
                    client_uuid_,
                    protocol::ClientTypes::CPP,
...
ClientConnectionManagerImpl::submit_connect_to_cluster_task()
{
    ...
    std::shared_ptr<client::impl::hazelcast_client_instance_impl> c =
...
{
    {
        ...
        connection->set_connected_server_version(response.server_hazelcast_version());
        ...
        if (local_address) {
            HZ_LOG(
              ...
              boost::str(
                ...
                response.server_hazelcast_version() % *local_address % *connection));
        } else {
            HZ_LOG(
              logger_,
              info,
...
void
ClientConnectionManagerImpl::connect_to_tpc_ports(std::shared_ptr<Connection> conn, std::vector<int> ports)
{
    auto connector = std::make_shared<tpc_channel_connector>(
        client_uuid_,
        conn,
        move(ports),
        *executor_,
...
ReadHandler::ReadHandler(Connection& connection, size_t buffer_size)
  ...
  , builder_(connection)
  , last_read_time_(std::chrono::steady_clock::time_point{})
{}
  ...
  const std::shared_ptr<spi::impl::ClientInvocation>& client_invocation)
{
    ...
    int partition_id = client_invocation->get_client_message()->get_partition_id();
    ...
    return tpc_channels_[channel_index]->async_write(shared_from_this(), client_invocation);
}
...
Connection::inner_close()
{
    ...
    if (tpc_channels_initialized_){
        boost::asio::post(
            get_executor(),
            [thisConnection](){
                for (auto& channel : thisConnection->get_tpc_channels())
                    channel->close();
            }
        );
    }
...
boost::asio::io_context::strand& Connection::get_executor()
{
    return socket_strand_;
}

tpc_channel_connector::tpc_channel_connector(boost::uuids::uuid client_uuid,
...
void tpc_channel_connector::initiate()
{
    ...
    HZ_LOG(
        ...
        boost::str(
            boost::format(
                "Initiating connection attempts to TPC channels running on ports %1% for %2%"
            ) % boost::algorithm::join(ports_str_vec, " ") % *connection_
        )
    );
    ...
    for (int port : tpc_ports_) {
        ...
        boost::asio::post(
            ...
            [self, host, port, index](){
                self->connect(host, port, index);
            }
        );
    }
}

void tpc_channel_connector::connect(std::string host, int port, int index)
{
    ...
    HZ_LOG(
        ...
        boost::str(
            boost::format(
                "Trying to connect to TPC channel on port %1% for %2%"
            ) % port % *connection_
        )
    );
    ...
    try
    {
        ...
        channel = channel_creator_.create(addr, connection_timeout_millis_, *connection_);
    ...
    catch (const std::exception& e)
    {
        HZ_LOG(
            ...
            boost::str(
                boost::format(
                    "Exception during the connection to attempt to TPC channel on port %1% for %2%: %3%"
...
bool tpc_channel_connector::connection_failed() const
{
    return failed_ || !connection_->is_alive();
}

void tpc_channel_connector::write_authentication_bytes(socket& channel)
{
    protocol::ClientMessage authentication_message {protocol::ClientMessage::REQUEST_HEADER_LEN};

    auto frame = authentication_message.begin_frame();
    frame.frame_len = protocol::ClientMessage::UUID_SIZE + protocol::ClientMessage::SIZE_OF_FRAME_LENGTH_AND_FLAGS;
    frame.flags = uint64_t(protocol::ClientMessage::UNFRAGMENTED_MESSAGE | protocol::ClientMessage::IS_FINAL_FLAG);
    ...
    if (!channel.write(move(buffer_without_header))) {
        BOOST_THROW_EXCEPTION(
            exception::hazelcast_ {
                boost::str(
                    boost::format(
                        "Cannot write authentication bytes to the TPC channel %1% for %2%"
                    ) % channel % *connection_
                )
            }
        );
...
void tpc_channel_connector::on_successful_channel_connection(std::unique_ptr<socket> channel, int index)
{
    {
        ...
        if (connection_failed()) {
            ...
            HZ_LOG(
                ...
                boost::str(
                    boost::format(
                        "Closing the TPC channel %1% for %2% as one of the connections is failed."
                    ) % *channel % *connection_
                )
            );
            on_failure(move(channel));
    ...
    if (remaining_.fetch_sub(1) == 1) {
        ...
        if (!connection_->is_alive()) {
            HZ_LOG(
                ...
                boost::str(
                    boost::format(
                        "Closing all TPC channel connections for %1% as the connection is closed."
                    ) % *connection_
                )
            );
...
void tpc_channel_connector::on_failure(std::unique_ptr<socket> channel)
{
    std::lock_guard<std::recursive_mutex> lock {mtx_};
...
void tpc_channel_connector::close_channel(std::unique_ptr<socket> channel)
{
    if (!channel) {
        return;
    }
    ...
    catch (const std::exception& e)
    {
        HZ_LOG(
            ...
            boost::str(
                boost::format(
                    "Exception while closing TPC channel %1%"
                ) % e.what()
            )
        );
    }
...
void tpc_channel_connector::close_all_channels()
{
    ...
    timer_ = clientExecutionService.schedule_with_repetition(
      [=]() {
          ...
          for (auto& connection :
              ...
              for (int i = 0; i < connection->get_tpc_channels().size(); ++i) {
                check(
                    connection,
                    *connection->get_tpc_channels()[i],
                    i,
                    now
                );
              }
...
)
{
    ...
    if (now - last_read_time > heartbeat_timeout_) {
        HZ_LOG(
            logger_,
            warning,
            boost::str(
...
boost::asio::io_context& SocketFactory::io_ctx()
{
    return io_;
}
