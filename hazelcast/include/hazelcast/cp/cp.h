...
public:
     ...
     * @return \code true\endcode if successful; or \code false\endcode if the
     ...
     *
     * @param function the function applied to the value, the value is not
     * changed
...
public:
     ...
     * @return \true if the new count was set, \false if the current count is
     * not zero
     ...
     * and <li>Countdown owner is set to \null.
     * </ul>
     ...
     *
     * @return true only if the internal counter has reached zero. This function
     * may spuriously return false with very low probability even if the
     * internal counter has reached zero.
     */
     ...
     *
     * Blocks the calling thread until the internal counter reaches 0 or server
     * thread is interrupted (e.g. cluster shutdown). If it is zero already,
     * returns immediately.
     *
...
public:
     ...
     * server side CPSubsystemConfig#getSessionTimeToLiveSeconds config. In this
     * case, its CP session will be closed on the corresponding CP group because
     * it could not commit session heartbeats in the meantime. After the user
     * pause wakes up again, the same thread attempts to acquire the lock
     ...
     * encounters a long pause, which is longer than
     * server config CPSubsystemConfig#getSessionTimeToLiveSeconds. In this
     * case, its CP session will be closed on the corresponding CP group because
 ...
 * stl \counting_semaphore (@see
 * https://en.cppreference.com/w/cpp/thread/counting_semaphore). Semaphores are
 * often used to restrict the number of threads than can access some physical or
 * logical resource. <p> \counting_semaphore is a cluster-wide counting
 * semaphore. Conceptually, it maintains a set of permits. Each \acquire blocks
 * if necessary until a permit is available, and then takes it. Dually, each
 * \release adds a permit, potentially releasing a blocking acquirer. However,
 * no actual permit objects are used; the semaphore just keeps a count of the
 * number available and acts accordingly. <p> Hazelcast's distributed semaphore
 * implementation guarantees that threads invoking any of the \acquire methods
 * are selected to obtain permits in the order of their invocations
 * (first-in-first-out; FIFO). Note that FIFO ordering implies the order which
 * the primary replica of an \counting_semaphore receives these acquire
 * requests. Therefore, it is possible for one member to invoke \acquire before
 * another member, but its request hits the primary replica after the other
 * member. <p> This class also provides convenience methods to work with
 * multiple permits at once. Beware of the increased risk of indefinite
 * postponement when using the multiple-permit acquire. If permits are released
 * one by one, a thread waiting for one permit will acquire it before a thread
 * waiting for multiple permits regardless of the call order. <p> Correct usage
 * of a semaphore is established by programming convention in the application.
 * <p>
 * \counting_semaphore works on top of the Raft consensus algorithm. It offers
 ...
 * available on at most one side of the partition. <p> It has 2 variations: <ul>
 * <li>
 ...
 * However, the session-aware version comes with a limitation, that is, a
 * HazelcastInstance cannot release permits before acquiring them first. In
 * other words, a Hazelcast client can release only the permits it has acquired
 * earlier. It means, you can acquire a permit from one thread and release it
 * from another thread using the same Hazelcast client, but not different
 * instances of Hazelcast client. You can use the session-aware CP
 * \counting_semaphore impl by disabling server side JDK compatibility server
 * SemaphoreConfig#setJDKCompatible(boolean). Although the session-aware impl
 * has a minor difference to the JDK Semaphore, we think it is a better fit for
 * distributed environments because of its safe auto-cleanup mechanism for
 * acquired permits.
 * </li>
 * <li>
 * The second impl is sessionless. This impl
 * does not perform auto-cleanup of acquired permits on failures. Acquired
 * permits are not bound to HazelcastInstance and permits can be released
 * without acquiring first. However, you need to handle failed permit owners on
 * your own. If a Hazelcast server or a client fails while holding some permits,
 * they will not be automatically released. You can use the sessionless CP
 * \counting_semaphore impl by enabling server side config
 ...
 * only 1 permit (similar to stl \binary_semaphore). In this case, the semaphore
 * works like a non-reentrant lock. <p> All of the API methods in the new CP
 * \fenced_lock impl offer the exactly-once execution semantics for the
 * session-aware version. For instance, even if a \release call is internally
 * retried because of a crashed Hazelcast member, the permit is released only
 * once. However, this guarantee is not given for the sessionless, a.k.a,
 * JDK-compatible CP \counting_semaphore. For this version, you can tune
 * execution semantics via server side config
...
public:
     ...
     * via server side SemaphoreConfig then a HazelcastInstance can only release
     * a permit which it has acquired before. In other words, a
     * HazelcastInstance cannot release a permit without acquiring it first. <p>
     * Otherwise, which means the underlying impl is the JDK compatible
     * Semaphore is configured via server side SemaphoreConfig, there is no
     * requirement that a HazelcastInstance that releases a permit must have
     * acquired that permit by calling one of the \acquire methods. A
     * HazelcastInstance can freely release a permit without acquiring it first.
     * In this case, correct usage of a semaphore is established by programming
     ...
     * @param rel_time the maximum time to wait for a permit. The supported
    ...
    /**
     * Same as \try_acquire_for except that the wait time is calculated from the
     * \abs_time point.
     */
    ...
    // --- extended methods
    /**
     * Tries to initialize this counting_semaphore instance with the given
     * permit count
     *
     ...
     * semaphore. <p> This method is typically used for debugging and testing
     * purposes.
     *
...
public:
     ...
     * cp_subsystem::get_atomic_long("myint64_t@group1"), the given group will
     * be initialized first, if not initialized already, and then the
     ...
     * cp_subsystem::get_atomic_reference("myRef@group1"), the given group will
     * be initialized first, if not initialized already, and then the
     ...
     * group. If a group name is given, like
     * cp_subsystem::get_atomic_reference("myLatch@group1"), the given group
     * will be initialized first, if not initialized already, and then the
     ...
     * cp_subsystem::get_atomic_reference("myLock@group1"), the given group will
     * be initialized first, if not initialized already, and then the
     * fenced_lock instance will be created on this group. Returned fenced_lock
     ...
     * be created on the DEFAULT CP group. If a group name is given, like
     * cp_subsystem::get_semaphore("mySemaphore@group1"), the given group will
     * be initialized first, if not initialized already, and then the semaphore
     * instance will be created on this group. Returned semaphore instance
