...
{
public:
    username_password_credentials(const std::string& name,
...
public:
    ...
    /**
     * Returns the configured cluster name. The name is sent as part of client
     * authentication message and may be verified on the member.
     *
    ...
    /**
     *  There are two types of credentials you can provide,
     * \username_password_credentials and \token_credentials
     *
     ...
     * simply because the member died. However it is not clear whether the
     * application is performed or not. For idempotent operations this is
     * harmless, but for non idempotent ones retrying can cause to undesirable
     * effects. Note that the redo can perform on any member.
     *
    ...
    /**
     * Will be called with the Socket, each time client creates a connection to
     * any Member.
     *
     * \return itself ClientConfig
     */
    ...
    /**
     * Adds a listener to configuration to be registered when hazelcast_client
     * starts. Warning 1: If listener should do a time consuming operation,
     * off-load the operation to another thread. otherwise it will slow down the
     * system.
     *
     ...
     *   * set version of portable classes in this client (@see
     ...
     * \return itself ClientConfig
     */
    client_config& set_property(const std::string& name,
     ...
     * \return the found config. If none is found, a default configured one is
     * returned.
     */
    const config::reliable_topic_config& get_reliable_topic_config(
     ...
     * @see NearCacheConfig
     */
    const config::near_cache_config* get_near_cache_config(
      const std::string& name) const;
     ...
     *
     * \param networkConfig {\link
     * com.hazelcast.client.config.client_network_config} to be set \return
    ...
    client_config& set_connection_strategy_config(
      const config::client_connection_strategy_config&
        connection_strategy_config);

    /**
     * Returns a {\link ClientFlakeIdGeneratorConfig} configuration for the
     * given flake ID generator name. <p> The name is matched by pattern to the
     * configuration and by stripping the partition ID qualifier from the given
     * {@code name}. If there is no config found by the name, it will return the
     * configuration with the name {@code "default"}.
     *
    ...
    find_flake_id_generator_config(const std::string& name);

    /**
     * Returns the {\link ClientFlakeIdGeneratorConfig} for the given name,
     * creating one if necessary and adding it to the collection of known
     * configurations. <p> The configuration is found by matching the the
     * configuration name pattern to the provided {@code name} without the
     * partition qualifier (the part of the name after {@code '@'}). If no
     * configuration matches, it will create one by cloning the
     ...
     * #addFlakeIdGeneratorConfig(ClientFlakeIdGeneratorConfig)}. <p> Because it
     * adds new configurations if they are not already present, this method is
     * intended to be used before this config is used to create a hazelcast
     * instance. Afterwards, newly added configurations may be ignored.
     *
     * \param name name of the flake ID generator config
     ...
     */
    const config::client_flake_id_generator_config*
    get_flake_id_generator_config(const std::string& name);

    /**
     * Adds a flake ID generator configuration. The configuration is saved under
     * the config name, which may be a pattern with which the configuration will
    ...
    /**
     * This feature reduces number of hops and increase performance for smart
     * clients. It is enabled by default for smart clients. This config has no
     * effect for unisocket clients.
     *
     * @param enabled enables client to get backup acknowledgements directly
     * from the member that backups are applied
     ...
     * @throws illegal_argument_exception if the {@code tpc_config} is {@code null}
     * @since 5.3
     */
...
private:
    ...
    const config::reliable_topic_config* lookup_reliable_topic_config(
      const std::string& name) const;
    ...
    std::unordered_map<std::string, config::reliable_topic_config>
      reliable_topic_config_map_;
    ...
    config::matcher::matching_point_config_pattern_matcher
      config_pattern_matcher_;
