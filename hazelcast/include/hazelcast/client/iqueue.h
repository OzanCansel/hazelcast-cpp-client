...
public:
    ...
    {
        std::unique_ptr<
          impl::item_event_handler<protocol::codec::queue_addlistener_handler>>
          itemEventHandler(new impl::item_event_handler<
                           protocol::codec::queue_addlistener_handler>(
            get_name(),
        ...
        return proxy::IQueueImpl::add_item_listener(std::move(itemEventHandler),
                                                    include_value);
    }
    ...
    /**
     *
     * @return the head of the queue. If queue is empty waits for an item to be
     * added.
     */
    template<typename E>
     ...
     * @return the head of the queue. If queue is empty waits for specified
     * time.
     */
    template<typename E>
     ...
     *
     * @return removes head of the queue and returns it to user . If not
     ...
     * Returns immediately without waiting.
     *
     * @return head of queue without removing it. If not available returns empty
    ...
    {
        return proxy::IQueueImpl::contains_all_data(
    ...
    /**
     *
     * Removes the elements from this queue that are not available in given
     * "elements" vector
     * @param elements std::vector<E>
