...
public:
     ...
     * get the value.
     * @param key
     * @return value value, if there is no mapping for key then returns
     * boost::none.
     */
     ...
     * @param key
     * @param value
     * @return the previous value. if there is no mapping for key, then returns
     * boost::none.
     */
     ...
     * @param ttl      maximum time for this entry to stay in the map. 0 means
     * infinite.
     * @return the previous value. if there is no mapping for key, then returns
     * boost::none.
     */
     ...
     * @return true if the entry with key and value exists and removed, false
     * otherwise
     */
    template<typename K, typename V>
     ...
     *
     * Note that calling this method also removes all entries from callers Near
     * Cache.
     *
     ...
     * @param key          key of the entry
     * @param value        value of the entry
     * @param ttl  maximum time for this entry to stay in the map in
    ...
    /**
     * Puts an entry into this map, if the specified key is not already
    ...
    */
     ...
     *
     * @param key key to lock.
     * @throws illegal_monitor_state if the current thread does not hold this
     * lock MTODO
     */
    template<typename K>
    ...
    /**
     * Adds an interceptor for this map. Added interceptor will intercept
     * operations and execute user defined methods and will cancel operations if
     * user defined method throw exception.
     *
     *
     * Interceptor should be serializable.
     * Notice that map interceptor runs on the nodes. Because of that same class
     * should be implemented in java side with same classId and factoryId.
     ...
     *                     contain the value.
     *
     * @return registrationId of added listener that can be used to remove the
     * entry listener.
     */
     ...
     *
     * @param listener     entry listener
     * @param predicate The query filter to use when returning the events to the
     * user.
     * @param includeValue <tt>true</tt> if <tt>EntryEvent</tt> should
     *                     contain the value.
     *
     ...
     * so changes to the map are <b>NOT</b> reflected in the vector, and
     * vice-versa.
     *
     ...
     * The vector is <b>NOT</b> backed by the map,
     * so changes to the map are <b>NOT</b> reflected in the collection, and
     * vice-versa.
     *
    ...
    /**
     * Returns a std::vector< std::pair<K, V> > clone of the mappings contained
     * in this map. The vector is <b>NOT</b> backed by the map, so changes to
     * the map are <b>NOT</b> reflected in the set, and vice-versa.
     *
     ...
     * threads. The Map is not blocked during this operation. <p> The time taken
     * in proportional to the size of the Map and the number Members. <p>
     * <b>Searches while indexes are being built</b>
     * <p>
     * Until the index finishes being created, any searches for the attribute
     * will use a full Map scan, thus avoiding using a partially built index and
    ...
    /**
     * Convenient method to add an index to this map with the given type and
     * attributes. Attributes are indexed in ascending order. <p>
     *
     ...
     * Returns the the ResultType which is result of the process() method of
     * EntryProcessor.
     *
     * EntryProcessor should be serializable.
     * Notice that map EntryProcessor runs on the nodes. Because of that, same
     * class should be implemented in java side with same classId and factoryId.
     *
     ...
     * collection of keys. Returns the results mapped by each key in the map.
     *
     *
     * Notice that map EntryProcessor runs on the nodes. Because of that, same
     * class should be implemented in java side
     *
     * @tparam ResultType that entry processor will return
     * @tparam EntryProcessor type of entry processor class
     ...
     * @tparam ResultType that entry processor will return
     * @tparam EntryProcessor type of entry processor class
     * @tparam predicate The filter to apply for selecting the entries at the
     * server side.
     * @param entryProcessor that will be applied
     */
    template<typename K,
