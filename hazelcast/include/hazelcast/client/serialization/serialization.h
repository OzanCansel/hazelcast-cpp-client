...
/**
 * This class represents the type of a Hazelcast serializable object. The fields
 * can take the following values:
 * 1. Primitive types: factoryId=-1, classId=-1, typeId is the type id for that
 * primitive as listed in
 * @link SerializationConstants
 * 2. Array of primitives: factoryId=-1, classId=-1, typeId is the type id for
 * that array as listed in
 * @link SerializationConstants
 * 3. identified_data_serializer: factory, class and type ids are non-negative
 * values as registered by the DataSerializableFactory.
 * 4. portable_serializer: factory, class and type ids are non-negative values.
 * 5. Custom serialized objects: factoryId=-1, classId=-1, typeId is the
 * non-negative type id as registered by custom_serializer.
 *
 */
...
/**
 * typed_data class is a wrapper class for the serialized binary data. It does
 * late deserialization of the data only when the get method is called.
 */
...
public:
    ...
    /**
     * Deserializes the underlying binary data and produces the object of type
     * T.
     *
     * <b>CAUTION</b>: The type that you provide should be compatible with what
     * object type is returned with the get_type API, otherwise you will either
     * get an exception of incorrectly try deserialize the binary data.
     *
...
/**
 * ClassDefinition defines a class schema for portable classes. It allows to
 * query field names, types, class id etc. It can be created manually using
 * {@link ClassDefinitionBuilder} or on demand during serialization phase.
 *
...
public:
     ...
     * @return true if this class definition contains a field named by given
     * name
     */
...
/**
 * ClassDefinitionBuilder is used to build and register ClassDefinitions
 * manually.
 *
 * @see ClassDefinition
...
/**
 * Provides a mean of reading portable fields from a binary in form of java
 * primitives arrays of java primitives , nested portable fields and array of
 * portable fields.
 */
...
public:
     ...
     *
     * Note that portable fields can not read after getRawDataInput() is called.
     * In case this happens, io will be thrown.
     *
     * @return rawDataInput
     * @throws io_exception
     */
...
/**
 * Provides a mean of writing portable fields to a binary in form of java
 * primitives arrays of java primitives , nested portable fields and array of
 * portable fields.
 */
...
public:
    ...
    /**
     * After writing portable fields, one can write remaining fields in old
     * fashioned way consecutively at the end of stream. User should not that
     * after getting rawDataOutput trying to write portable fields will result
     * in io_exception
     *
