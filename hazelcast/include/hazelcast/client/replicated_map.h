 ...
 * <p>Whenever a value is written asynchronously, the new value will be
 * internally distributed to all existing cluster members, and eventually every
 * node will have the new value.</p> <p>When a new node joins the cluster, the
 * new node initially will request existing values from older nodes and
 * replicate them locally.</p>
 *
 *
 */
...
public:
        ...
        return proxy::ReplicatedMapImpl::add_entry_listener(
          std::shared_ptr<impl::BaseEventHandler>(
            new EntryEventHandler<
              protocol::codec::replicatedmap_addentrylistener_handler>(
              get_name(),
     ...
     * <p/>
     * <p><b>Warning:</b></p>
     * This method uses <tt>hashCode</tt> and <tt>equals</tt> of the binary form
     * of the <tt>key</tt>, not the actual implementations of <tt>hashCode</tt>.
     *
    ...
    /**
     * Adds an continuous entry listener for this map. The listener will be
     * notified for map add/remove/update/evict events filtered by the given
     * predicate.
     *
     * @param listener  the entry listener to add
    ...
    typename std::enable_if<std::is_base_of<query::predicate, P>::value,
                            boost::future<boost::uuids::uuid>>::type
    add_entry_listener(entry_listener&& listener,
                       const P& predicate,
                       const K& key)
    {
        return controlled_serialization(
    ...
    /**
     * Due to the lazy nature of the returned array, changes to the map
     * (addition, removal, update) might be reflected on the collection.<br/>
     * Changes on the map are <b>NOT</b> reflected on the collection on the
     * <b>CLIENT</b> or vice versa. The order of the elements is not guaranteed
     * due to the internal asynchronous replication behavior. If a specific
     * order is needed, use
     ...
     * Due to the lazy nature of the returned array, changes to the map
     * (addition, removal, update) might be reflected on the array.<br/> Changes
     * on the map are <b>NOT</b> reflected on the set on the <b>CLIENT</b> or
     * vice versa. The order of the elements is not guaranteed due to the
     * internal asynchronous replication behavior.<br/> Changes to any returned
     * object are <b>NOT</b> replicated back to other members.
     *
     ...
     * @return The previous value if the key existed in the map or null pointer
     * otherwise.
     */
    template<typename K, typename V, typename R = V>
...
private:
    ...
    public:
        ...
        {
            if (event_type ==
                static_cast<int32_t>(entry_event::type::CLEAR_ALL)) {
    ...
    private:
          ...
          int32_t number_of_affected_entries)
        {
            ...
            if (old_value) {
                oldVal =
                  typed_data(std::move(*old_value), serialization_service_);
            }
            if (merging_value) {
                mergingVal =
                  typed_data(std::move(*merging_value), serialization_service_);
            }
            if (key) {
