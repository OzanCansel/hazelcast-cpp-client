  ...
  : public impl::BaseEventHandler
{
     ...
     * @param member_infos List of member infos  at the cluster associated with
     * the given version params:
     */
    virtual void handle_membersview(
      int32_t version,
      std::vector<member> const& member_infos) = 0;

    /**
     * @param version Incremental state version of the partition table
     * @param partitions The partition table. In each entry, it has uuid of the
     * member and list of partitions belonging to that member
     */
...
/**
 * Destroys the proxy given by its name cluster-wide. Also, clears and releases
 * all resources of this proxy.
 */
ClientMessage HAZELCAST_API
 ...
 *
 * The second parameter, the clientAttribute is a String that is composed of
 * key=value pairs separated by ','. The following characters ('=' '.' ',' '\')
 * should be escaped.
 *
 * Please note that if any client implementation can not provide the value for
 * statistics, the corresponding key, value pair will not be presented in the
 * statistics string. Only the ones, that the client can provide will be added.
 *
 * The third parameter, metrics is a compressed byte array containing all
 * metrics recorded by the client.
 *
 ...
 *   - set of enum:            excluded targets
 * [MANAGEMENT_CENTER,JMX,DIAGNOSTICS]
 *   - set of <string,string>: tags associated with the metric
 *
 * The used compression algorithm is the same that is used inside the IMDG
 * clients and members for storing the metrics blob in-memory. The algorithm
 * uses a dictionary based delta compression further deflated by using ZLIB
 * compression.
 *
 ...
 * Size of dictionary blob: the size of the ZLIB compressed blob as it is
 ...
 * ===================
 *
 * The dictionary is built from the string fields of the metric and assigns an
 * int dictionary id to every string in the metrics in the blob. The dictionary
 * is serialized to the dictionary blob sorted by the strings using the
 * following layout.
 *
 * +------------------------------------------------+--------------------+
 ...
 * forever.This method returns a clone of the previous value, not the original
 * (identically equal) value previously put into the map.Time resolution for TTL
 * is seconds. The given TTL value is rounded to the next closest second value.
 */
ClientMessage HAZELCAST_API
 ...
 * operation). Returns the value to which this map previously associated the
 * key, or null if the map contained no mapping for the key. If this map permits
 * null values, then a return value of null does not necessarily indicate that
 * the map contained no mapping for the key; it's also possible that the map
 * explicitly mapped the key to null. The map will not contain a mapping for the
...
/**
 * Replaces the the entry for a key only if existing values equal to the
 * testValue
 */
ClientMessage HAZELCAST_API
...
/**
 * Removes the mapping for a key from this map if existing value equal to the
 * this value
 */
ClientMessage HAZELCAST_API
...
/**
 * Removes the mapping for a key from this map if it is present.Unlike
 * remove(Object), this operation does not return the removed value, which
 * avoids the serialization cost of the returned value.If the removed value will
 * not be used, a delete operation is preferred over a remove operation for
 * better performance. The map will not contain a mapping for the specified key
 * once the call returns. This method breaks the contract of EntryListener. When
 * an entry is removed by delete(), it fires an EntryEvent with a null oldValue.
 * Also, a listener with predicates will have null values, so only keys can be
 * queried via predicates
 */
ClientMessage HAZELCAST_API
 ...
 * by calling MapStore.storeAll() and/or MapStore.deleteAll().
 */
ClientMessage HAZELCAST_API
map_flush_encode(const std::string& name);

/**
 * Tries to remove the entry with the given key from this map within the
 * specified timeout value. If the key is already locked by another thread
 * and/or member, then this operation will wait the timeout amount for acquiring
 * the lock.
 */
ClientMessage HAZELCAST_API
...
/**
 * Tries to put the given key and value into this map within a specified timeout
 * value. If this method returns false, it means that the caller thread could
 * not acquire the lock for the key within the timeout duration, thus the put
...
/**
 * Same as put except that MapStore, if defined, will not be called to
 * store/persist the entry. If ttl is 0, then the entry lives forever.
 */
ClientMessage HAZELCAST_API
...
/**
 * Puts an entry into this map with a given ttl (time to live) value if the
 * specified key is not already associated with a value. Entry will expire and
 * get evicted after the ttl.
 */
ClientMessage HAZELCAST_API
 ...
 * Puts an entry into this map with a given ttl (time to live) value.Entry will
 * expire and get evicted after the ttl If ttl is 0, then the entry lives
 * forever. Similar to the put operation except that set doesn't return the old
 ...
 * released.If the lock is not available then the current thread becomes
 * disabled for thread scheduling purposes and lies dormant until the lock has
 * been acquired. Scope of the lock is this map only. Acquired lock is only for
 * the key in this map. Locks are re-entrant, so if the key is locked N times
 * then it should be unlocked N times before another thread can acquire it.
 */
ClientMessage HAZELCAST_API
 ...
 * time.After lease time, the lock will be released.If the lock is not
 * available, then the current thread becomes disabled for thread scheduling
 * purposes and lies dormant until one of two things happens the lock is
...
/**
 * Checks the lock for the specified key.If the lock is acquired then returns
...
/**
 * Releases the lock for the specified key. It never blocks and returns
 * immediately. If the current thread is the holder of this lock, then the hold
 * count is decremented.If the hold count is zero, then the lock is released. If
 * the current thread is not the holder of this lock, then ILLEGAL_MONITOR_STATE
 * is thrown.
 */
ClientMessage HAZELCAST_API
 ...
 * and execute user defined methods and will cancel operations if user defined
  ...
  : public impl::BaseEventHandler
{
     ...
     * @param number_of_affected_entries Number of entries affected by this
     * event.
     */
...
/**
 * Adds a MapListener for this map. To receive an event, you should implement a
 * corresponding MapListener sub-interface for that event.
 */
ClientMessage HAZELCAST_API
...
/**
 * Removes the specified entry listener. If there is no such listener added
 * before, this call does no change in the cluster and returns false.
 */
ClientMessage HAZELCAST_API
 ...
 * This method returns a clone of original mapping, modifying the returned value
 * does not change the actual value in the map. One should put modified value
 * back to make changes visible to all nodes.
 */
ClientMessage HAZELCAST_API
...
/**
 * Evicts the specified key from this map. If a MapStore is defined for this
 * map, then the entry is not deleted from the underlying MapStore, evict only
 * removes the entry from the memory.
 */
ClientMessage HAZELCAST_API
...
/**
 * Evicts all keys from this map except the locked ones. If a MapStore is
 * defined for this map, deleteAll is not called by this method. If you do want
 * to deleteAll to be called use the clear method. The EVICT_ALL event is fired
...
/**
 * Returns a set clone of the keys contained in this map. The set is NOT backed
 * by the map, so changes to the map are NOT reflected in the set, and
 * vice-versa. This method is always executed by a distributed query, so it may
 * throw a QueryResultSizeExceededException if query result size limit is
 * configured.
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns the entries for the given keys. If any keys are not present in the
 * Map, it will call loadAll The returned map is NOT backed by the original map,
 * so changes to the original map are NOT reflected in the returned map, and
 * vice-versa. Please note that all the keys in the request should belong to the
 * partition id to which this request is being sent, all keys matching to a
 * different partition id shall be ignored. The API implementation using this
 * request may need to send multiple of these request messages for filling a
 * request for a key set if the keys belong to different partitions.
 */
ClientMessage HAZELCAST_API
 ...
 * The collection is NOT backed by the map, so changes to the map are NOT
 * reflected in the collection, and vice-versa. This method is always executed
 * by a distributed query, so it may throw a QueryResultSizeExceededException if
 * query result size limit is configured.
 */
ClientMessage HAZELCAST_API
...
/**
 * Queries the map based on the specified predicate and returns the keys of
 * matching entries. Specified predicate runs on all members in parallel.The set
 * is NOT backed by the map, so changes to the map are NOT reflected in the set,
 * and vice-versa. This method is always executed by a distributed query, so it
 * may throw a QueryResultSizeExceededException if query result size limit is
 * configured.
 */
ClientMessage HAZELCAST_API
 ...
 * matching entries.Specified predicate runs on all members in parallel. The
 * collection is NOT backed by the map, so changes to the map are NOT reflected
 * in the collection, and vice-versa. This method is always executed by a
 * distributed query, so it may throw a QueryResultSizeExceededException if
 * query result size limit is configured.
 */
ClientMessage HAZELCAST_API
...
/**
 * Queries the map based on the specified predicate and returns the matching
 * entries.Specified predicate runs on all members in parallel. The collection
 * is NOT backed by the map, so changes to the map are NOT reflected in the
 * collection, and vice-versa. This method is always executed by a distributed
 * query, so it may throw a QueryResultSizeExceededException if query result
 ...
 * put(Object,Object) put(k, v) on this map once for each mapping from key k to
 * value v in the specified map.The behavior of this operation is undefined if
 * the specified map is modified while the operation is in progress. Please note
 * that all the keys in the request should belong to the partition id to which
 * this request is being sent, all keys matching to a different partition id
 * shall be ignored. The API implementation using this request may need to send
 * multiple of these request messages for filling a request for a key set if the
...
/**
 * This method clears the map and invokes MapStore#deleteAll deleteAll on
 * MapStore which, if connected to a database, will delete the records from that
 * database. The MAP_CLEARED event is fired for any registered listeners. To
 * clear a map without calling MapStore#deleteAll, use #evictAll.
 */
ClientMessage HAZELCAST_API
 ...
 * Returns the the object which is result of the process() method of
 * EntryProcessor.
 */
ClientMessage HAZELCAST_API
...
/**
 * Applies the user defined EntryProcessor to the entry mapped by the key.
 * Returns immediately with a Future representing that task.EntryProcessor is
 * not cancellable, so calling Future.cancel() method won't cancel the operation
 * of EntryProcessor.
 */
ClientMessage HAZELCAST_API
...
/**
 * Applies the user defined EntryProcessor to the all entries in the map.Returns
...
/**
 * Applies the user defined EntryProcessor to the entries in the map which
 * satisfies provided predicate. Returns the results mapped by each key in the
 * map.
 */
ClientMessage HAZELCAST_API
 ...
 * collection of keys.The results mapped by each key in the collection.
 */
ClientMessage HAZELCAST_API
 ...
 * always successfully unlocks the key, never blocks,and returns immediately.
 */
ClientMessage HAZELCAST_API
...
/**
 * Adds listener to map. This listener will be used to listen near cache
 * invalidation events.
 */
ClientMessage HAZELCAST_API
  ...
  : public impl::BaseEventHandler
{
     ...
     * @param partition_uuid UUID of the source partition that invalidated entry
     * belongs to.
     ...
     * @param partition_uuids List of UUIDs of the source partitions that
     * invalidated entries belong to.
     * @param sequences List of sequence numbers of the invalidation events.
     */
    virtual void handle_imapbatchinvalidation(
      std::vector<serialization::pimpl::data> const& keys,
 ...
 * function on that entry until all entries have been processed in the targetted
 * partition or the function throws an exception.
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns the collection of values associated with the key. The collection is
 * NOT backed by the map, so changes to the map are NOT reflected in the
...
/**
 * Returns the set of keys in the multimap.The collection is NOT backed by the
 * map, so changes to the map are NOT reflected in the collection, and
 * vice-versa.
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns the collection of values in the multimap.The collection is NOT backed
 * by the map, so changes to the map are NOT reflected in the collection, and
 * vice-versa.
 */
ClientMessage HAZELCAST_API
 ...
 * backed by the map, so changes to the map are NOT reflected in the collection,
 * and vice-versa
 */
ClientMessage HAZELCAST_API
 ...
 * notified for all add/remove/update/evict events for the specified key only.
 */
ClientMessage HAZELCAST_API
...
/**
 * Adds an entry listener for this multimap. The listener will be notified for
 * all multimap add/remove/update/evict events.
 */
ClientMessage HAZELCAST_API
...
/**
 * Acquires the lock for the specified key for the specified lease time. After
 * the lease time, the lock will be released. If the lock is not available, then
 * the current thread becomes disabled for thread scheduling purposes and lies
 * dormant until the lock has been acquired. Scope of the lock is for this map
 * only. The acquired lock is only for the key in this map.Locks are re-entrant,
 * so if the key is locked N times, then it should be unlocked N times before
 ...
 * After lease time, the lock will be released. If the lock is not available,
 * then the current thread becomes disabled for thread scheduling purposes and
 * lies dormant until one of two things happens:the lock is acquired by the
...
/**
 * Checks the lock for the specified key. If the lock is acquired, this method
...
/**
 * Removes all the entries with the given key. The collection is NOT backed by
 * the map, so changes to the map are NOT reflected in the collection, and
 * vice-versa.
 */
ClientMessage HAZELCAST_API
 ...
 * collection c may result in elements being in neither, either or both
 * collections when the associated exception is thrown. Attempts to drain a
 * queue to itself result in ILLEGAL_ARGUMENT. Further, the behavior of this
 ...
 * add elements to collection may result in elements being in neither, either or
...
/**
 * Removes the specified item listener. If there is no such listener added
 * before, this call does no change in the cluster and returns false.
 */
ClientMessage HAZELCAST_API
 ...
 * Integer.MAX_VALUE if there is no intrinsic limit. Note that you cannot always
 * tell if an attempt to insert an element will succeed by inspecting
 * remainingCapacity because it may be the case that another thread is about to
 ...
 * onMessage() function of the given MessageListener is called. More than one
 * message listener can be added on one instance.
 */
ClientMessage HAZELCAST_API
  ...
  : public impl::BaseEventHandler
{
     ...
     * @param uuid UUID of the member that dispatches this event.
     */
    virtual void handle_topic(serialization::pimpl::data const& item,
                              int64_t publish_time,
                              boost::uuids::uuid uuid) = 0;
};

/**
 * Stops receiving messages for the given message listener.If the given listener
 ...
 * it is unchanged. Returns true if this list contained the specified element
...
/**
 * Adds an item listener for this collection. Listener will be notified for all
 * collection add/remove events.
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns a view of the portion of this list between the specified from,
 * inclusive, and to, exclusive.(If from and to are equal, the returned list is
 ...
 * returned list.(Structural modifications are those that change the size of
 * this list, or otherwise perturb it in such a fashion that iterations in
 ...
 * operation). If this set already contains the element, the call leaves the set
 ...
 * refuse to add any particular element, including null, and throw an exception,
 * as described in the specification for Collection Individual set
 * implementations should clearly document any restrictions on the elements that
 * they may contain.
 */
ClientMessage HAZELCAST_API
set_add_encode(const std::string& name,
               const serialization::pimpl::data& value);

/**
 * Removes the specified element from this set if it is present (optional
 * operation). Returns true if this set contained the element (or equivalently,
 ...
 * is the union of the two sets. The behavior of this operation is undefined if
 * the specified collection is modified while the operation is in progress.
 */
ClientMessage HAZELCAST_API
...
/**
 * Cancels the task running on the member that owns the partition with the given
 * id.
 */
ClientMessage HAZELCAST_API
 ...
 */
ClientMessage HAZELCAST_API
atomicref_get_encode(const cp::raft_group_id& group_id,
                     const std::string& name);
 ...
 * cluster. If there is an old value, it will be replaced by the specified one
 * and returned from the call. In addition, you have to specify a ttl and its
 * TimeUnit to define when the value is outdated and thus should be removed from
 * the replicated map.
 */
ClientMessage HAZELCAST_API
 ...
 * contains no mapping for the key. If this map permits null values, then a
...
replicatedmap_putall_encode(
  const std::string& name,
  const std::vector<std::pair<serialization::pimpl::data,
                              serialization::pimpl::data>>& entries);

/**
 * The clear operation wipes data out of the replicated maps.It is the only
 * synchronous remote operation in this implementation, so be aware that this
 * might be a slow operation. If some node fails on executing the operation, it
 * is retried for at most 3 times (on the failing nodes only). If it does not
 * work after the third time, this method throws a OPERATION_TIMEOUT back to the
 * caller.
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns a lazy Set view of the key contained in this map. A LazySet is
 * optimized for querying speed (preventing eager deserialization and hashing on
 * HashSet insertion) and does NOT provide all operations. Any kind of mutating
 * function will throw an UNSUPPORTED_OPERATION. Same is true for operations
 * like java.util.Set#contains(Object) and
 * java.util.Set#containsAll(java.util.Collection) which would result in very
 * poor performance if called repeatedly (for example, in a loop). If the use
 * case is different from querying the data, please copy the resulting set into
...
/**
 * Adds a near cache entry listener for this map. This listener will be notified
 * when an entry is added/removed/updated/evicted/expired etc. so that the near
...
/**
 * Returns the value for the specified key, or null if this map does not contain
 * this key.
 */
ClientMessage HAZELCAST_API
 ...
 * specified value. The object to be put will be accessible only in the current
 * transaction context till transaction is committed.
 */
ClientMessage HAZELCAST_API
 ...
 * Associates the specified value with the specified key in this map. If the map
 * previously contained a mapping for the key, the old value is replaced by the
 * specified value. This method is preferred to #put(Object, Object) if the old
 * value is not needed. The object to be set will be accessible only in the
 * current transaction context until the transaction is committed.
 */
ClientMessage HAZELCAST_API
 ...
 * with the given value. The object to be put will be accessible only in the
 * current transaction context until the transaction is committed.
 */
ClientMessage HAZELCAST_API
 ...
 * object to be replaced will be accessible only in the current transaction
 * context until the transaction is committed.
 */
ClientMessage HAZELCAST_API
...
/**
 * Removes the mapping for a key from this map if it is present. The map will
 * not contain a mapping for the specified key once the call returns. The object
 * to be removed will be accessible only in the current transaction context
...
/**
 * Removes the mapping for a key from this map if it is present. The map will
 * not contain a mapping for the specified key once the call returns. This
 * method is preferred to #remove(Object) if the old value is not needed. The
 * object to be deleted will be removed from only the current transaction
 * context until the transaction is committed.
 */
ClientMessage HAZELCAST_API
...
/**
 * Removes the entry for a key only if currently mapped to a given value. The
 * object to be removed will be removed from only the current transaction
 * context until the transaction is committed.
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns a collection clone of the values contained in this map. The
 * collection is NOT backed by the map, so changes to the map are NOT reflected
 * in the collection, and vice-versa. This method is always executed by a
 * distributed query, so it may throw a QueryResultSizeExceededException if
 * query result size limit is configured.
...
/**
 * Returns number of items in the ringbuffer. If no ttl is set, the size will
 * always be equal to capacity after the head completed the first looparound the
 ...
 * where the items are added to. The initial value of the tail is -1.
 */
ClientMessage HAZELCAST_API
 ...
 * where the oldest items in the ringbuffer are found. If the RingBuffer is
 * empty, the head will be one more than the tail. The initial value of the head
 * is 0 (1 more than tail).
 */
ClientMessage HAZELCAST_API
...
/**
 * Returns the remaining capacity of the ringbuffer. The returned value could be
 * stale as soon as it is returned. If ttl is not set, the remaining capacity
...
/**
 * Adds an item to the tail of the Ringbuffer. If there is space in the
 * ringbuffer, the call will return the sequence of the written item. If there
 * is no space, it depends on the overflow policy what happens: OverflowPolicy
 * OVERWRITE we just overwrite the oldest item in the ringbuffer and we violate
 * the ttl OverflowPolicy FAIL we return -1. The reason that FAIL exist is to
 * give the opportunity to obey the ttl. <p/> This sequence will always be
 * unique for this Ringbuffer instance so it can be used as a unique id
 * generator if you are publishing items on this Ringbuffer. However you need to
 * take care of correctly determining an initial id when any node uses the
 * ringbuffer for the first time. The most reliable way to do that is to write a
 * dummy item into the ringbuffer and use the returned sequence as initial  id.
 * On the reading side, this dummy item should be discard. Please keep in mind
 * that this id is not the sequence of the item you are about to publish but
...
/**
 * Reads one item from the Ringbuffer. If the sequence is one beyond the current
 * tail, this call blocks until an item is added. This method is not destructive
 * unlike e.g. a queue.take. So the same item can be read by multiple readers or
 * it can be read multiple times by the same reader. Currently it isn't possible
 * to control how long this call is going to block. In the future we could add
 * e.g. tryReadOne(long sequence, long timeout, TimeUnit unit).
 */
ClientMessage HAZELCAST_API
ringbuffer_readone_encode(const std::string& name, int64_t sequence);

/**
 * Adds all the items of a collection to the tail of the Ringbuffer. A addAll is
 * likely to outperform multiple calls to add(Object) due to better io
 * utilization and a reduced number of executed operations. If the batch is
 * empty, the call is ignored. When the collection is not empty, the content is
 * copied into a different data-structure. This means that: after this call
 * completes, the collection can be re-used. the collection doesn't need to be
 * serializable. If the collection is larger than the capacity of the
 * ringbuffer, then the items that were written first will be overwritten.
 * Therefor this call will not block. The items are inserted in the order of the
 * Iterator of the collection. If an addAll is executed concurrently with an add
 * or addAll, no guarantee is given that items are contiguous. The result of the
 * future contains the sequenceId of the last written item
 */
ClientMessage HAZELCAST_API
...
/**
 * Reads a batch of items from the Ringbuffer. If the number of available items
 * after the first read item is smaller than the maxCount, these items are
 * returned. So it could be the number of items read is smaller than the
 * maxCount. If there are less items available than minCount, then this call
 * blacks. Reading a batch of items is likely to perform better because less
 * overhead is involved. A filter can be provided to only select items that need
 * to be read. If the filter is null, all items are read. If the filter is not
 * null, only items where the filter function returns true are returned. Using
 * filters is a good way to prevent getting items that are of no value to the
 * receiver. This reduces the amount of IO and the number of operations being
 * executed, and can result in a significant performance improvement.
 */
ClientMessage HAZELCAST_API
 ...
 */
ClientMessage HAZELCAST_API
experimental_authentication_encode(
